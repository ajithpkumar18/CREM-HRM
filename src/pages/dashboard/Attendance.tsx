...
export default function Attendance() {
    ...
    const fetchAttendance = async () => {
        ...
        try {
            ...
            setEmployee(response.data.attendanceRecords); // Update the employee state with the fetched data
        } catch (err: any) {
    ...
    useEffect(() => {
        fetchAttendance();
    }, [date]); // Refetch attendance data when the date changes

    const handleOpenModal = (time: string) => {
        setSelectedCheckInTime(time);
        setSelectedTimeIndex(null); // Adjust as needed if index is required
        setShowModal(true);
    };
